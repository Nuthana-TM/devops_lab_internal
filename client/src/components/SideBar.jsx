    ...
    >
      ...
      <div className='flex flex-col '>
        {filteredUsers.map((user, index) => (
          <div 
          onClick={() => {setSelectedUser(user);setUnseenMessages({...unseenMessages,[user._id]:0})}}//To achieve dynamic keys, JavaScript requires computed property syntax:thats why we use [user._id]
          key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer mx-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            <img
